 ...
 * finalizeResult()). WEB_SCENARIOS.PROJECT_ID NOT NULL olduğu için projectId bilinmeyen (ör. proje
...
export async function createScenario(input: CreateScenarioInput): Promise<{ id: number }> {
  return withConnection(async (connection) => {
    const result = await connection.execute<{ id: number[] }>(
      `INSERT INTO WEB_SCENARIOS (PROJECT_ID, SCENARIO_NAME, SCENARIO_TEXT, TARGET_URL, CREATED_BY)
       VALUES (:projectId, :scenarioName, :scenarioText, :targetUrl, :createdBy)
       RETURNING SCENARIO_ID INTO :id`,
      {
        projectId: input.projectId,
        // WEB_SCENARIOS.SCENARIO_NAME VARCHAR2(200) — çağıran taraf (LegacyTestService) bu sınıra
