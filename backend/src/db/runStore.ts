 ...
 * SCENARIOS satırı + TEK bir RUNS satırı oluşturulur (mevcut JSON davranışıyla birebir aynı
...
export async function createRun(input: CreateRunInput): Promise<{ id: number }> {
  return withConnection(async (connection) => {
    const result = await connection.execute<{ id: number[] }>(
      `INSERT INTO RUNS (SCENARIO_ID, PROJECT_ID, STATUS, BROWSER_ENGINE, STARTED_AT, FINISHED_AT, STARTED_BY, STEPS_JSON)
