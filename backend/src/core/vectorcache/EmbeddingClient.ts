/** Ollama'nın embedding üretmesi bu süreden uzun sürerse iptal edilir. */
  ...
  async embed(text: string): Promise<number[]> {
    ...
    try {
      ...
      if (err instanceof Error && err.name === 'AbortError') {
        throw new Error(
          `Ollama (${this.baseUrl}) ${EMBED_TIMEOUT_MS}ms içinde yanıt vermedi. Ollama çalışıyor mu ` +
            "kontrol edin ('ollama serve').",
        );
      }
      throw new Error(
